...
@InternalAdaptyApi
public class FlexStackElement internal constructor(
    internal val condition: List<Condition>,
    internal val direction: FlexElement.Direction,
    internal val hStackElement: HStackElement,
    internal val vStackElement: VStackElement,
    ...
    private fun RenderDirected(
        ...
        when (direction) {
            FlexElement.Direction.HORIZONTAL -> hStackElement.toComposable(dispatch, modifier).invoke()
            FlexElement.Direction.VERTICAL -> vStackElement.toComposable(dispatch, modifier).invoke()
        }
    }
}
