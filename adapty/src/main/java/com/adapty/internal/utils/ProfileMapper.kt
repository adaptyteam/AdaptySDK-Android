...
internal class ProfileMapper(
    ...
    fun map(dto: ProfileDto) =
        AdaptyProfile(
            ...
            appliedExternalAttributionProviders = mapExternalAttributionProviders(dto.appliedExternalAttributionProviders),
    ...
    private fun mapExternalAttributionProviders(providers: List<String?>?) =
        providers?.mapNotNull { provider -> provider?.let(::AdaptyExternalAttributionProvider) }
            .orEmpty().immutableWithInterop()
