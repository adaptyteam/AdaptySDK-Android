package com.adapty.internal.data.serialization.extractors
...
internal class SingleVariationExtractor(
    ...
    override fun extract(jsonElement: JsonElement): JsonElement? {
        ...
        val variation = dataObject.getAsJsonObject(attributesKey) ?: dataObject
        ...
        return extractInternal(variation)
    }
    ...
    private fun extractInternal(jsonObject: JsonObject): JsonElement? {
        ...
        if (jsonObject.has("flow_id")) {
            jsonObject.requires("flow_name") { "name in Flow should not be null" }

            (jsonObject.get(flowPaywallsKey) as? JsonArray)?.forEach { paywallElement ->
                ((paywallElement as? JsonObject)?.get("products") as? JsonArray)
                    ?.forEachIndexed { index, productElement ->
                        (productElement as? JsonObject)?.let { product ->
                            if (!product.has("paywall_product_index"))
                                product.addProperty("paywall_product_index", index)
                        }
                    }
            }
        } else if (jsonObject.has("onboarding_id")) {
    ...
    private companion object {
        ...
        const val flowPaywallsKey = "variations"
    }
}